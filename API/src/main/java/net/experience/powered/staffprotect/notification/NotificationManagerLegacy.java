...
public class NotificationManagerLegacy {

    private final NotificationBus bus;
    private final JavaPlugin plugin;

    public NotificationManagerLegacy(final @NotNull JavaPlugin plugin, final @NotNull NotificationBus bus) {
        this.bus = bus;
        this.plugin = plugin;
    }
     ...
     * @deprecated Deprecated in favor of {@link NotificationManagerLegacy#sendMessage(Component)}
     */
    @Deprecated(forRemoval = true)
     ...
     */
    @Contract(" -> new")
    public static @NotNull NotificationManagerLegacy getInstance() {
        final StaffProtect api = StaffProtect.getInstance();
        return new NotificationManagerLegacy(api.getPlugin(), api.getNotificationBus());
    }
}
